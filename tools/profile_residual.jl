...
bytes_by_type = Dict{String, Int}();
cnt_by_type = Dict{String, Int}()
...
for a in allocs.allocs
    ...
    for fr in a.stacktrace
        s = string(fr.func)
        if !occursin("Profile", s) && !occursin("Allocs", s)
            site = "$(fr.func) @ $(fr.file):$(fr.line)"
            break
        end
    end
...
for (ty, b) in first(sort(collect(bytes_by_type); by=kv->-kv[2]), 12)
    println(
        lpad(round(100b/tot; digits=1), 5),
        "%  ",
        lpad(b, 12),
        "  ",
        lpad(get(cnt_by_type, ty, 0), 7),
        "  ",
        ty
    )
end
...
io = IOBuffer();
Profile.print(
    IOContext(io, :displaysize=>(120, 200)); format=:flat, sortedby=:count, mincount=50
)
