...
for (l, src) in CASES
    interp = try
        ...
        String["ORACLE-ERR:" * first(sprint(showerror, e), 30)]
    end
    ev = try
        r = MC.compile_run(src)
        isempty(r.answers) ? String[] : norm(r.answers[end][2])
    catch e
        String["ERR:" * first(sprint(showerror, e), 30)]
    end
    mk = try
...
println(
    rpad("CASE", 14),
    rpad("INTERPRETER", 22),
    rpad(":eval (arrow 5)", 22),
    rpad(":mork (ARROW 6)", 22),
    "vs INTERP"
)
println("="^100)
...
println(
    "query is NotReducible and returns itself. Compare the :eval column to tell decline from defect."
)
