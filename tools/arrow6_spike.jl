...
const MC = MeTTaCore
const IV=MC.Eval;
const IF=MC.CompilerFrontend;
const IA=MC.CompilerANormal;
const IE=MC.CompilerEmit
ask(l, f) =
    try
        print(l, " -> ")
        println(f())
    catch e
        println(l, " -> RAISED: ", first(replace(sprint(showerror, e), "\n"=>" | "), 240))
    end
_parse(sp, t) = begin
    toks=IV.tokenize(t)
    i=Ref(1)
...
println("="^72);
println("STEP 0 — does Emit.jl's MM2 actually RUN on a CoreSpace?");
println("="^72)
SRC = "(= (inc \$n) (+ \$n 1))\n"
sp = IV.Space();
...
ask(
    "1 A-normalize",
    () -> (
        global cls = IA.translate_program(IF.lower_program(_parse(sp, SRC)));
        "clauses=$(length(cls))"
    )
)
ask(
    "2 emit_program",
    () -> (
        global r = IE.emit_program(cls);
        "fields=$(propertynames(r))  emitted=$(r.emitted)"
    )
)
ask(
    "2b the emitted MM2",
    () -> begin
        for f in propertynames(r)
            v=getproperty(r, f)
            v isa AbstractVector{<:AbstractString} && return join(v, "\n     ")
        end
...
ask(
    ...
    () -> begin
        ...
        for f in propertynames(r)
            v=getproperty(r, f)
            v isa AbstractVector{<:AbstractString} && (rules=v; break)
        end
        ...
        for rule in rules
            MC.space_add_all_sexpr!(cs.inner, rule)
        end
