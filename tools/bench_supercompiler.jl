...
using MeTTaCore; const MC = MeTTaCore

function rand_dag(nn, ne)
    st = UInt64(0x12345678ABCDEF01); es = Set{String}()
    while length(es) < ne
        st = st*6364136223846793005 + 1442695040888963407; i = Int(st>>33)%nn
        st = st*6364136223846793005 + 1442695040888963407; j = Int(st>>33)%nn
        i==j && continue; i,j = minmax(i,j); push!(es, "(edge $i $j)")
    end
    join(es, "\n")
end
...
for ne in (30, 60, 90)
    f = rand_dag(ne÷2, ne)
    sd = new_core_space(); MC.space_add_all_sexpr!(sd.inner, f); MC.space_add_all_sexpr!(sd.inner, PROG)
    td = bench(()->MC.space_metta_calculus!(sd.inner, 50_000)); nd = np3(sd)
    st = new_core_space(); MC.space_add_all_sexpr!(st.inner, f)
    tt = bench(()->sc_execute!(st, PROG; opts=SCOptions(max_steps=50_000))); nt = np3(st)
    sp = tt>0 ? round(td/tt; digits=2) : NaN
    println("$ne | $(round(td;digits=3)) $(round(tt;digits=3)) $(sp)x | $nd $nt $(nd==nt ? "OK" : "MISMATCH")")
end
