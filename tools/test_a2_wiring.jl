...
SM.load_metta!(
    sp, read(joinpath(@__DIR__, "..", "lib", "MOSES", "utilities.metta"), String)
)

lst(n) = "(" * join(("e$i" for i in 1:n), " ") * ")"
bench(expr, reps) = (SM.load_metta!(sp, expr); @elapsed for _ in 1:reps
    SM.load_metta!(sp, expr)
end) / reps

for n in (10, 20)
    expr = "!(concatT $(lst(n)) $(lst(n)))"
    SM.A2_ENABLED[] = false
    roff = SM.load_metta!(sp, expr)
    SM.A2_ENABLED[] = true
    ron = SM.load_metta!(sp, expr)
    ok = string(roff) == string(ron)
    SM.A2_ENABLED[] = false
    toff = bench(expr, 3) * 1e3   # ms
    SM.A2_ENABLED[] = true
    ton = bench(expr, 200) * 1e3 # ms (native is fast → more reps)
    println(
        "n=$n  match=$(ok ? "✓" : "✗")  interp=$(round(toff;digits=1))ms  native=$(round(ton;digits=4))ms  speedup=$(round(toff/ton))×"
    )
    ok || println("   off=", roff, "\n   on =", ron)
end
