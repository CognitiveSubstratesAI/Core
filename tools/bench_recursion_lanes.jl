...
function peano(n::Int)
    t = "Z"; for _ in 1:n; t = "(S " * t * ")"; end; t
end
function mklist(xs)
    t = "Nil"; for x in reverse(xs); t = "(Cons $x " * t * ")"; end; t
end
...
function l4_space(eqprog::String)
    sp = IN.Space(); IN.load_core_stdlib!(sp); IN.load_metta!(sp, eqprog); sp
end
function run_L4(eqprog::String, goal::String; tabled::Bool = false)
    IN.untable_all!(); IN._table_reset!()
    sp = l4_space(eqprog)
...
function cell(; lane, wl, n, rep, ok, t, bytes, gcpct, status, note = "")
    ROWS[] += 1
    println("CELL lane=$lane wl=$wl n=$n rep=$rep ok=$(ok ? 1 : 0) t=$(round(t, digits = 6)) ",
...
function timed_cell(; lane, wl, n, thunk, oracle::Union{Nothing,Set{String}}, note = "")
    bs = Int[]; ts = Float64[]; gs = Float64[]; halt = :none
    for rep in 1:REPS
        GC.gc()
        tm = Base.@timed thunk()
        ans = tm.value
        ...
        push!(bs, tm.bytes); push!(ts, tm.time); push!(gs, gp)
        extra = note
    ...
    keep = bs[2:end]; kt = ts[2:end]; kg = gs[2:end]
    spread = 100 * (maximum(kt) - minimum(kt)) / minimum(kt)
    println("SUM lane=$lane wl=$wl n=$n bytes=$keep identical=$(length(unique(keep)) == 1) ",
...
function phase_correctness()
    ...
    for (wl, eqp, ilp, goal, oracle) in wls
        ...
        println("  L4T  answers=$(a4t)  ok=$(Set(a4t) == oracle)  auto_table!=(tabled=$(ti.tabled), skipped=$(ti.skipped))")
        # L1
        a1, st1 = run_L1(eqp, goal)
        println("  L1   $(st1 === :REJECTED ? "REJECTED-BY-GATE (served=[], bang deferred)" : "answers=$(a1)  ok=$(Set(a1) == oracle)  set==L4U:$(Set(a1) == Set(a4u))")")
        # L2 (never diverges: one semi-naive fixpoint)
        a2 = run_L2(ilp, goal)
        println("  L2   answers=$(a2)  ok=$(Set(a2) == oracle)  set==L4U:$(Set(a2) == Set(a4u))")
        # L3 — skip fib (proven non-terminating; measured in a child process instead)
        if wl == "fib"
            println("  L3   NOT RUN IN-SESSION — non-terminating (see DIVERGE child-process cell)")
        else
            a3 = run_L3(ilp, goal)
            println("  L3   answers=$(a3)  ok=$(Set(a3) == oracle)  set==L4U:$(Set(a3) == Set(a4u))  subset_of_L4U:$(issubset(Set(a3), Set(a4u)))")
        end
        ROWS[] += 1
    end
    ...
    spx = IN.Space(); IN.load_core_stdlib!(spx); IN.load_metta!(spx, raw"(= (g a) hit)")
    ...
    for n in 0:12, m in 0:6
        ...
        r == peano_oracle(n, m) || (nfail += 1; nfail <= 5 && println("   MISMATCH n=$n m=$m got=$r"))
    end
...
function phase_gates()
    ...
    for (wl, eqp, goal) in [("fib", FIB_EQ, "(fib 10)"),
        ...
        for (bang, f) in forms
            (bang || MC.mm2_head(f) != "=") && continue
            a = MC.mm2_expr_args(f); lhs, rhs = a[2], a[3]
            println("  rule $(f)")
            println("     mm2_is_relational  [:95]  = ", MC.mm2_is_relational(f))
            println("     mm2_is_arith_body  [:318] = ", MC.mm2_is_arith_body(f))
            println("     GATE(4) head_below_top(LHS) [:474] = ", MC._mm2_head_below_top(lhs, heads))
            println("     GATE(4) head_below_top(RHS) [:475] = ", MC._mm2_head_below_top(rhs, heads))
    ...
    for (wl, eqp, goal) in [("fib", FIB_EQ, "(fib 10)"), ("peano", PEANO_EQ, peano_goal(2, 1)),
                            ("append", APP_EQ, app_goal(3))]
        cs = MC.new_core_space()
        r = MC.mc_run(cs, "", eqp * "\n!" * goal)
...
function phase_l3steps()
    ...
    for k in 0:5
        ...
        i = findfirst("(fib ", s)
        if i === nothing; println("  no nested (fib …) at step $k"); break; end
        j = first(i); d = 0; e = j
        while e <= lastindex(s)
            s[e] == '(' && (d += 1); s[e] == ')' && (d -= 1)
            d == 0 && break
    ...
    for (r, d) in [(raw"(= (f $n) (- $n 1))", "(f 5)"), (raw"(= (f $n) $n)", "(f (- 5 1))"),
...
function phase_peano()
    ...
    for n in [4, 8, 16, 32, 64, 128, 256]
        g = peano_goal(n, 2); o = Set([peano_oracle(n, 2)])
...
function phase_append()
    ...
    stop = Set{String}()
    for n in [4, 8, 16, 32, 64, 128, 256]
        g = app_goal(n); o = Set([app_oracle(n)])
...
function phase_fib()
    ...
    IN.untable_all!(); IN._table_reset!()
    sp = l4_space(FIB_EQ); ti = IN.auto_table!(sp)
    println("  auto_table! -> tabled=$(ti.tabled) skipped=$(ti.skipped)")
    for rep in 1:4
        tm = Base.@timed IN.load_metta!(sp, "!(fib 24)")
        println("  warm-hit rep=$rep t=$(round(tm.time, digits = 6)) bytes=$(tm.bytes) ans=$(tm.value)")
    end
    ROWS[] += 1
end
...
function bench_main()
    ...
    flush(stdout)
    PHASE == "correctness" ? phase_correctness() :
    PHASE == "gates"       ? phase_gates()       :
    PHASE == "l3steps"     ? phase_l3steps()     :
    PHASE == "peano"       ? phase_peano()       :
    PHASE == "append"      ? phase_append()      :
    PHASE == "fib"         ? phase_fib()         :
    PHASE == "l1ref"       ? phase_l1ref()       :
