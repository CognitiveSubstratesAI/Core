...
for f in files
    ...
    cls = try
        IA.translate_program(IF.lower_program(_parse(sp, read(f, String))))
    catch
        continue
    end
    r = try
...
println("="^76)
println(
    "emitted=$emitted  declined=$declined  (",
    round(100*emitted/max(1, emitted+declined); digits=1),
    "% emitted)"
)
println("-"^76)
for (k, v) in sort(collect(hist); by=last, rev=true)
    println(
        ...
        lpad(v, 5),
        "  ",
        lpad(round(100v/max(1, declined); digits=1), 5),
        "%"
    )
end
println("="^76)
println(
    "Top row = the fragment that buys the most coverage next. Build against THAT, not the total."
)
