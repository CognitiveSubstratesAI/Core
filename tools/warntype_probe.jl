...
probe(f, args, label) = begin
    io = IOBuffer()
    try code_warntype(io, f, Tuple{map(typeof,args)...}) catch e; print(io, "ERR $e"); end
    s = String(take!(io))
    ...
    println("── $label : ::Any=$(length(collect(eachmatch(r"::Any\b", s))))  Box=$boxes  Union{=$unions")
    for ln in split(s, '\n')
...
println("  slots       :: ", fieldtype(Bindings, :slots), "   (isconcretetype: ", isconcretetype(fieldtype(Bindings,:slots)), ")")
println("  Expression.children :: ", fieldtype(Expression, :children), "   (eltype concrete: ", isconcretetype(eltype(fieldtype(Expression,:children))), ")")
