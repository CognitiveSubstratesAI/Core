...
MC.space_add_all_sexpr!(cs.inner, DRINK_DB)
...
for (dollar, wild, pre, exp) in cases
    ...
    @printf(
        "  %-24s interp=%d scan=%d native=%d counter=%s  %s\n",
        dollar,
        i,
        s,
        n,
...
function bench(nsubj)
    ...
    MC.space_add_all_sexpr!(csb.inner, d)
    ...
    rd()
    sc()                                        # warmup
    tr = @elapsed for _ in 1:100000
        rd()
    end
    ts = @elapsed for _ in 1:10
        sc()
    end
    @printf(
        "  N=%6d | counter-read %6.3f us | scan %8.2f ms | speedup %9.0fx | support=%d\n",
