...
graph organized into the world-model **Spaces**, all co-resolving on one key —
the neuron `root_id` — with the raw data kept immutable as evidence. The
conceptual App+Common mapping is in `docs/CONNECTOME_MULTISPACE.md`; this page
documents the implemented substrate.
