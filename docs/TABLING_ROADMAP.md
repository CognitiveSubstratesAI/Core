...
| **0.2** | ⚠️ **RESTATED 2026-08-16 — the `()` cause was WRONG and is now VOID.** `IR.jl:133` already replaced the `:Nil` sentinel with `const UNIT_HEAD = Symbol("()")`; VERIFIED: `()` round-trips both ways and `(= (f) ())` compiles to `(= (f) (function (return ())))`. **THE REAL DEFECT IS IN A-NORMALIZATION:** it hoists an expression-headed DATA TUPLE into a `chain` as if it were a call — emitted IL shows `(chain (() () $list2) $__t1 …)`, where `chain`'s first argument is the EVALUATED position and must be an instruction. `(() () $list2)` is `foldl-atom`'s ACCUMULATOR — an argument, not a subexpression. ⚠️ Wrapping it in `metta` is what 08-11 did and it fails identically (`metta` would apply `()` to arguments), so **NO `_instr` treatment can work** — three attempts (08-11 call, 08-15 data, 08-16 re-run) failed for this one reason. | Blocks the `\|-` three-arm fix = **16 of 18 compile-arrow declines, 13 of them PLN inference rules**. | fix the hoist in `ANormal.jl` so an expression-headed data tuple in ARGUMENT position stays in place; THEN the middle-clause arm in `EmitIL._instr` is a one-predicate change. Proved corpus must move exactly `remove-857` and `overlap-857`. |
