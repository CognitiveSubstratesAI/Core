...
| **0.2** | 🔴 **RESTATED TWICE ON 2026-08-16 — the cause moved TWO stages.** (a) The `()` → `(Nil)` cause is **VOID**: `IR.jl:133` already has `const UNIT_HEAD = Symbol("()")` and `()` round-trips. (b) The `_instr` cause is **VOID**: three attempts (08-11 call-form, 08-15 data-form, 08-16 re-run) failed identically because A-normal HOISTED a data tuple into `chain`'s evaluated position — no lowering can repair that. (c) **THE ACTUAL BLOCKER IS A MISSING CONCEPT: BINDERS.** Adding PeTTa's middle clause to `ANormal` (expression-headed ⇒ DATA) is correct and fixes the hoist — and then `overlap-857` compiles far enough to emit `(function (unify ($left $intersection $right) $accum …))` with **the `foldl-atom` call GONE and `$accum`/`$elem` FREE**. `foldl-atom`/`map-atom`/`filter-atom` bind variables in their TEMPLATE arguments; `_KEEP_WHOLE` knows only `chain`. Same shape as 2.0: **a correct change exposing a masked defect** — the hoist was causing a DECLINE, and the decline was SAFE. | Blocks the `\|-` three-arm fix = **16 of 18 compile-arrow declines, 13 of them PLN inference rules**. ⚠️ `overlap-857` is declined TODAY, which is safe; fixing the middle clause ALONE converts a safe decline into a WRONG ANSWER. | teach `ANormal` that certain call ARGUMENTS are templates whose variables the callee binds (a binder-arity table: head ⇒ template arg indices) — NOT another `_KEEP_WHOLE` entry, since this is a per-ARGUMENT property. Naive widening already "cost MM2 four clauses" (`EmitIL.jl:735`). THEN the middle clause, THEN the `_instr` arm. Corpus must move exactly `remove-857` + `overlap-857`. |
