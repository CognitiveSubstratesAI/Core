...
io = IOBuffer()
Profile.print(
    IOContext(io, :displaysize => (100000, 320)); format=:flat, sortedby=:count, mincount=10
)
prof = String(take!(io))
...
for ln in split(prof, '\n')
    ...
    k = if occursin("PathMap/src", ln)
        "PathMap"
    elseif occursin("MorkSupercompiler/src", ln)
        "MorkSupercompiler"
    elseif occursin("MORK/src/kernel", ln)
        "MORK kernel (calculus/space)"
    elseif occursin("MORK/src/expr", ln)
        "MORK expr (unify/apply)"
    elseif occursin("MORK/src/frontend", ln)
        "MORK frontend (parse/ser)"
    elseif occursin("MORK/src", ln)
        "MORK other"
    elseif occursin("Core/src", ln)
        "Core"
    else
        "other/Base/runtime"
    end
    B[k] += c
    length(top) < 20 &&
        occursin(r"PathMap/src|MORK/src|MorkSupercompiler/src|Core/src", ln) &&
        push!(top, strip(ln))
end
tot = sum(values(B))
println(
    "\n=== MORK-lane eval self-time by area (saturate→MorkSupercompiler→MORK; total ",
    tot,
    ") ==="
)
for (k, c) in sort(collect(B); by=x -> -x[2])
    println("  ", rpad(k, 30), lpad(c, 8), "  (", round(100c/max(tot, 1); digits=1), "%)")
end
println("\n=== top project frames ===")
for t in top[1:min(end, 16)]
    println("  ", t)
end
