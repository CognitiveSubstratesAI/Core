...
for e in FAST
    ...
    extra =
        r.n_errors > 0 ? "  " * join(first(r.errors, min(2, length(r.errors))), " | ") : ""
...
let sp = first(ECANLiveHarness.fresh())
    for s in
        ["!(add-atom &self (AV a 0.8 0.1 0.0))", "!(add-atom &self (AV b 0.7 0.1 0.0))",
        "!(hebbian-update! a b)"]
        I.load_metta!(sp, s)
    end
    after = I.load_metta!(
        sp,
        "!(collapse (match &self (AsymHebbianLink a b \$s \$c) (AsymHebbianLink a b \$s \$c)))"
    )
    repr = isempty(after) ? "[]" : string(after[1])
    println(
        "  link after hebbian-update! = $repr  => ",
        occursin("AsymHebbianLink", repr) ? "CREATE_OK" : "CREATE_FAIL"
    )
    # update path: a second call should blend the weight, not duplicate
    I.load_metta!(sp, "!(hebbian-update! a b)")
    n = length(
        I.load_metta!(sp, "!(collapse (match &self (AsymHebbianLink a b \$s \$c) found))")
    )
    println(
        "  links after 2nd update (expect 1 row, no dup) = $n  => ",
        n == 1 ? "NO_DUP_OK" : "DUP_FAIL"
    )
end
...
let sp = first(ECANLiveHarness.fresh())
    ...
    println(
        "  get-link-weight x y after set = ",
        string.(w),
        "  => ",
        string(w) == "Any[0.42]" || occursin("0.42", string(w)) ? "SET_OK" : "SET_FAIL"
    )
end
println("VERIFY_DONE")
