...
#
# All tests run on the Minimal evaluator (the 234/234 hyperon-faithful engine) —
...
_qerrs(rs) = filter(r -> r isa Expression && !isempty(r.children) && r.children[1] == Sym("Error"), rs)
...
let sp = Space()
    load_core_stdlib!(sp)
    assert_harness_discriminates(e -> load_metta!(sp, e), rs -> !isempty(_qerrs(rs)); label = "quantale/Minimal")
end
...
@testset "Quantale foundation — Minimal-native" begin
    @testset "Q_prob (scalar [0,1], ×, max, 1) — quantale axioms" begin
        ...
        """)
        @test isempty(_qerrs(rs)); @test length(rs) == 3
    end
    ...
    @testset "Q_PLN PAIRED (n+,n-) — §4.4.1 / §9 cases 5-6  ⭐" begin
        ...
        """)
        @test isempty(_qerrs(rs)); @test length(rs) == 4
    end
    ...
    @testset "weakness morphism additivity — CoDD recurrence σ(⊗ p q)=σp+σq+1 (§6.1.2)" begin
        ...
        """)
        @test isempty(_qerrs(rs)); @test length(rs) == 1
    end
