...
@testset "GSLT presentation — s-expr surface" begin
  ...
  """
    ...
    @testset "RHOLANG — (Bind x Name) / (x)Proc plus freshness-guarded extrusion" begin
        ...
        @test length(p.equations[1].conditions) == 1 &&
            p.equations[1].conditions[1].var == :x
    ...
    @testset "COMPOUND categories — the arity language the port restored" begin
        ...
        p = _lang(
            "(language L (types Proc) (terms (: Bundle (-> (list Proc) Proc)) (: K (-> (arrow Proc Proc) Proc))))"
        )
    ...
    @testset "a FIRST-ORDER theory parses unchanged" begin
        p = _lang(
            "(language Monoid (types Elem) (terms (: Mult (-> Elem Elem Elem)) (: One (-> Elem))))"
        )
        @test !_PP.is_lambda_theory(p)
    ...
    @testset "MALFORMED presentations are REJECTED" begin
        ...
        @test_throws ErrorException _lang(
            "(language X (types T) (terms (: A (-> Bogus T))))"
        )
        @test_throws ErrorException _lang(
            "(language X (types T) (terms (: A (-> (list Bogus) T))))"
        )   # via a compound
        @test_throws ErrorException _lang(
            "(language X (types T) (terms (: A (-> (bind x Bogus) T))))"
        ) # via a binder
        @test_throws ErrorException _lang(
            "(language X (types T) (terms (: A (-> (bind x) T))))"
        )       # binder arity
        @test_throws ErrorException _lang(
    ...
    @testset "absent sections are empty, not an error" begin
        ...
        @test isempty(p.terms) && isempty(p.equations) && isempty(p.rewrites) &&
            isempty(p.literals)
        @test _PP.ddl_rung(p) == 1
    end
    ...
    @testset "types are concrete — no Any" begin
        p = _lang(
            "(language Lambda (types Term) (terms (: Lam (-> (bind x Term) (scope x Term) Term))))"
        )
