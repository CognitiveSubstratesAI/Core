...
@testset "LangDefPack — reflectable HE small-step rule table" begin
    ...
    @testset "table shape (14 rules: 10 live + 4 quiescent)" begin
        ...
        @test pack.language_id == "HE" && pack.granularity == "small-step" && pack.schema_version == 2
        @test pack.rules[1].name == "HES_GroundedDispatch" && pack.rules[1].claim == "rule-sound"
    end
    ...
    @testset "digest EXCLUDES claim (evidence ≠ content), INCLUDES provenance" begin
        ...
        bumped = repl(6, MC.LangDefRule(r6.name, r6.rule_id, r6.profiles, r6.provenance, "rule-sound", r6.live))
        @test langdef_digest("HE", "he-extended", "small-step", 2, bumped) == pack.source_digest
        # change provenance ⇒ digest CHANGES
        provd = repl(6, MC.LangDefRule(r6.name, r6.rule_id, r6.profiles, "DIFFERENT", r6.claim, r6.live))
        @test langdef_digest("HE", "he-extended", "small-step", 2, provd) != pack.source_digest
    end
