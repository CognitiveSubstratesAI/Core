...
function _ae(g, e; atol=1e-9)
    ...
    e isa AbstractVector && g isa AbstractVector && return length(g)==length(e) && all(_ae(gi,ei;atol=atol) for (gi,ei) in zip(g,e))
    g == e
end
...
@testset "NumpyOps — numpy-equivalence gate (helpers_test.metta + derived)" begin
    @testset "vec→scalar (✓ helpers_test)" begin
        ...
        @test _ae(qn("!(product (1 2 3 4))"), 24.0);        @test _ae(qn("!(product ())"), 1.0)
        @test _ae(qn("!(mean (1 2 3 4))"), 2.5)
        @test _ae(qn("!(variance (1 2 3 4))"), 1.25);       @test _ae(qn("!(variance (10))"), 0.0)
        @test _ae(qn("!(variance (2 2 2))"), 0.0);          @test _ae(qn("!(variance (1.0 2.0))"), 0.25)   # ddof=0
        @test _ae(qn("!(std (1 2 3 4))"), 1.12)                                                            # round-2
        @test _ae(qn("!(calculateNormDifference (1 2 3) (4 5 6))"), 27.0);  @test _ae(qn("!(calculateNormDifference () ())"), 0.0)
        @test _ae(qn("!(dotProduct (1 2 3) (4 5 6))"), 32.0); @test _ae(qn("!(dotProduct (1 0) (0 1))"), 0.0); @test _ae(qn("!(dotProduct () ())"), 0.0)
    end
    @testset "vec→vec (✓ helpers_test)" begin
        @test _ae(qn("!(listDifference (7 6 13) (4 5 6))"), Any[3.0,1.0,7.0]); @test _ae(qn("!(listDifference () ())"), Any[])
        @test _ae(qn("!(normalizeVector (3 4))"), Any[0.6,0.8]);               @test _ae(qn("!(normalizeVector ())"), Any[])
        @test _ae(qn("!(softmax (1 2 3))"), Any[0.09003057317038046,0.24472847105479764,0.6652409557748218]; atol=1e-10)
    end
    @testset "✓ scalar + split-free helpers_test" begin
        @test _ae(qn("!(roundNum 2.67777 2)"), 2.68);  @test _ae(qn("!(roundNum 0.0 2)"), 0.0)
    end
    @testset "⊘ derived from numpy convention (NOT from Julia output)" begin
        ...
        @test _ae(qn("!(positivePart -0.4)"), 0.0);   @test _ae(qn("!(positivePart 0.7)"), 0.7)
        @test _ae(qn("!(sigmoidNumber 0.0)"), 0.5)
        @test _ae(qn("!(absNumber -3.0)"), 3.0);      @test _ae(qn("!(expNumber 0.0)"), 1.0)
