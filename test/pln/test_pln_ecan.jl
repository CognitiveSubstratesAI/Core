...
let sp = Space()
    ...
    assert_harness_discriminates(
        e -> load_metta!(sp, e), rs -> !isempty(_eerrs(rs)); label="PLN/Minimal"
    )
end
...
@testset "§4.9 PLN↔ECAN co-working — demand as backward attention" begin
    ...
    @testset "T2 budget normalize — Σ STI = target, conserved" begin
        ...
        rs = load_metta!(
            sp, "!(assertEqual (get-sti X) 250.0)\n!(assertEqual (get-sti Y) 750.0)"
        )
        @test isempty(_eerrs(rs))
        @test length(rs) == 2
    end
    ...
    @testset "T4 bounded convergence — Σ STI stays at budget under iteration" begin
        ...
        load_metta!(
            sp,
            "(= (total-sti) (let \$ids (get-all-av-atoms) (foldl-atom \$ids 0.0 \$acc \$id (+ \$acc (get-sti \$id)))))"
        )
        load_metta!(sp, "!(set-sti! P 0.2)")
        load_metta!(sp, "!(set-sti! Q 0.2)")
        load_metta!(sp, "!(add-atom &self (dem P 0.8))")
        ...
        load_metta!(sp, "!(couple-demand->attention!)")
        rs = load_metta!(
            sp, "!(assertEqual (and (< 999.0 (total-sti)) (< (total-sti) 1001.0)) True)"
        )
        @test isempty(_eerrs(rs))
        @test length(rs) == 1
    end
...
"""
        ...
        rs = load_metta!(
            sp,
            "!(assertEqual (< 0.0 (get-sti B)) True)\n!(assertEqual (< 0.0 (get-sti A)) True)"
        )
        @test isempty(_eerrs(rs))
        @test length(rs) == 2
    end
