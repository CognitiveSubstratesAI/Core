# Validates the minimal-MeTTa stack machine (src/standard/Eval.jl) against the
...
only_result(atom) =
    (rs=bare_eval(atom); @assert length(rs) == 1 "got $(length(rs)) results: $rs"; rs[1])
...
@testset "Minimal MeTTa: cons/decons/unify (minimal-metta.md)" begin
    ...
    @test only_result(
        E(S("unify"), E(a, V("b")), E(V("a"), b), E(V("a"), V("b")), S("fail"))
    ) == E(a, b)
...
@testset "Minimal MeTTa: eval (minimal-metta.md §eval)" begin
    ...
    @test ev(E(S("eval"), E(PLUS, E(PLUS, Grounded(1), Grounded(2)), Grounded(3)))) ==
        S("NotReducible")
end
...
@testset "Minimal MeTTa: chain / function / return (minimal-metta.md)" begin
    ...
    @test only_result(
        E(S("chain"), E(S("eval"), E(PLUS, Grounded(2), Grounded(3))), V("x"),
            E(S("eval"), E(PLUS, Grounded(1), V("x"))))
    ) == Grounded(6)

    sp = Space(Atom[E(S("="), E(S("foo")), E(S("bar"))), E(S("="), E(S("bar")), S("a"))])
    ev(x) = (rs=bare_eval(x, sp); @assert length(rs) == 1 "got $(length(rs)): $rs"; rs[1])
    # (chain (eval (foo)) $x (eval $x)) → a ;  (chain (eval (foo)) $x (R $x)) → (R (bar))
    @test ev(E(S("chain"), E(S("eval"), E(S("foo"))), V("x"), E(S("eval"), V("x")))) ==
        S("a")
    @test ev(E(S("chain"), E(S("eval"), E(S("foo"))), V("x"), E(S("R"), V("x")))) ==
        E(S("R"), E(S("bar")))

    # function/return: (= (foo)(eval (bar)))(= (bar)(eval (baz)))(= (baz)(return a)); (function (eval (foo)))→a
    sp2 = Space(
        Atom[E(S("="), E(S("foo")), E(S("eval"), E(S("bar")))),
    ...
    rs = bare_eval(
        E(S("function"), E(S("eval"), E(S("div"), Grounded(35), Grounded(5), Grounded(0)))),
        spd
    )
