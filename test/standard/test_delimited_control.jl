...
_dc_parse(src::AbstractString, sp) = (toks = _DC.tokenize(src); _DC.parse_from(toks, Ref(1), sp.tokens))
_dc_root(goal::Atom) = Tuple{_DC.Frame,_DC.Bindings}[
    (_DC.Frame(goal, _DC.collect_vars(goal), nothing, _DC.no_handler, false, 0), _DC.Bindings())]
...
function _dc_run_capturing(goal::Atom, space)
    ...
    while !isempty(plan)
        f, fb = pop!(plan)
        if !f.finished && f.atom isa Expression && length((f.atom::Expression).children) >= 2 &&
        ...
        for (nf, nb) in _DC.interpret_stack(f, fb, space)
            (nf.finished && nf.prev === nothing) ? push!(out, (nf.atom, nb)) : push!(plan, (nf, nb))
        end
    end
    (out, caught)
end
...
@testset "delimited control — capture and resume over the CPS frame chain (§1.0)" begin
    ...
    @testset "capture copies its Bindings — entries unchanged across resumptions" begin
        s = Space(); load_core_stdlib!(s)
        load_metta!(s, raw"(= (g (F $x)) (Result $x))  (= (mark) (F M1))  (= (mark) (F M2))")
        goal = _DC._metta(_dc_parse("(g (mark))", s), _DC.UNDEF)
    ...
    @testset "Dependency carries source, continuation and target" begin
        ...
        goal = _DC._metta(_dc_parse("(g (mark))", s), _DC.UNDEF)
        (_, caught) = _dc_run_capturing(goal, s)
        src = _dc_parse("(mark)", s); tgt = _dc_parse("(g (mark))", s)
