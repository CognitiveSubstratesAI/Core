...
function _register_atom_ops!(eval_fn::Function)
    ...
    MORK.register_grounded!(
        ...
        args -> begin
            ...
            if startswith(s, "(") && endswith(s, ")")
                tokens = MeTTaCore._tokenise(s[2:(end - 1)])
                (idx < 0 || idx >= length(tokens)) &&
                    return "(Error (index-atom) IndexOutOfBounds)"
    ...
    MORK.register_grounded!(
        ...
        args -> begin
            ...
            if startswith(s, "(") && endswith(s, ")")
                nums = filter(
                    !isnothing, tryparse.(Float64, MeTTaCore._tokenise(s[2:(end - 1)]))
                )
    ...
    MORK.register_grounded!(
        ...
        args -> begin
            length(args) < 2 && return "()"
            t1 = if startswith(strip(args[1]), "(")
                MeTTaCore._tokenise(strip(args[1])[2:(end - 1)])
            else
                [args[1]]
            end
            t2 = if startswith(strip(args[2]), "(")
                MeTTaCore._tokenise(strip(args[2])[2:(end - 1)])
            else
                [args[2]]
            end
