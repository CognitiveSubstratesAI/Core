...
_cno_int(a::Atom) =
    (a isa Grounded && a.value isa Number && isinteger(a.value)) ? Int(a.value) : nothing
...
function _register_core_numeric_ops!()
    ...
    R["norm"] = _cno(
        ...
        xs -> (
            v=_cno_fvec(xs[1]);
            v===nothing ? nothing : Grounded(isempty(v) ? 0.0 : sqrt(sum(abs2, v)))
        )
    )
    R["sum"] = _cno(
        "sum",
        1,
        xs -> (v=_cno_fvec(xs[1]); v===nothing ? nothing : Grounded(sum(v; init=0.0)))
    )
    R["product"] = _cno(
        "product",
        1,
        xs -> (v=_cno_fvec(xs[1]); v===nothing ? nothing : Grounded(prod(v; init=1.0)))
    )
    R["mean"] = _cno(
        "mean",
        1,
        xs -> (
            ...
            if v===nothing||isempty(v)
                (v===nothing ? nothing : Grounded(0.0))
            else
                Grounded(_cno_mean(v))
            end
        )
    )
    R["variance"] = _cno(
    ...
    R["std"] = _cno(
        ...
        xs -> (
            ...
            if v===nothing||isempty(v)
                ...
                Grounded(round(sqrt(_cno_var(v)); digits=2))
            end
        )
    )
    # (vec,vec) → scalar
    R["calculateNormDifference"] = _cno(
        "calculateNormDifference",
        2,
        xs -> (
            a=_cno_fvec(xs[1]);
            b=_cno_fvec(xs[2]);
            if (a===nothing||b===nothing||length(a)!=length(b))
                nothing
            else
                Grounded(sum(abs2, a .- b; init=0.0))
            end
        )
    )
    R["dotProduct"] = _cno(
        ...
        xs -> (
            ...
            if (a===nothing||b===nothing||length(a)!=length(b))
                ...
                Grounded(isempty(a) ? 0.0 : sum(a .* b))
            end
        )
    )
    # (vec,vec) → vec
    R["listDifference"] = _cno(
        "listDifference",
        2,
        xs -> (
            a=_cno_fvec(xs[1]);
    ...
    R["vectorAdd"] = _cno(
        ...
        xs -> (
            ...
            if (a===nothing||b===nothing||length(a)!=length(b))
                ...
                _cno_wrapv(round.(a .+ b; digits=8))
            end
        )
    )
    R["averageArrays"] = _cno(
        "averageArrays",
        2,
        xs -> (
            a=_cno_fvec(xs[1]);
    ...
    R["normalizeVector"] = _cno(
        ...
        xs -> (
            ...
            if v===nothing
                ...
                (n=(isempty(v) ? 0.0 : sqrt(sum(abs2, v))); _cno_wrapv(n>0 ? v ./ n : v))
            end
        )
    )
    R["softmax"] = _cno(
    ...
    R["scaleArray"] = _cno(
        ...
        xs -> (
            v=_cno_fvec(xs[1]);
            s=_cno_num(xs[2]);
            (v===nothing||s===nothing) ? nothing : _cno_wrapv(v .* s)
        )
    )
    R["clipVector"] = _cno(
        "clipVector",
        ...
        xs -> (
            ...
            lo=_cno_num(xs[2]);
            hi=_cno_num(xs[3]);
            if (v===nothing||lo===nothing||hi===nothing)
                nothing
            else
                _cno_wrapv(clamp.(v, lo, hi))
            end
        )
    )
    R["roundList"] = _cno(
        ...
        xs -> (
            v=_cno_fvec(xs[1]);
            d=_cno_int(xs[2]);
            (v===nothing||d===nothing) ? nothing : _cno_wrapv(round.(v; digits=d))
        )
    )
    # scalar → scalar
    R["positivePart"] = _cno(
        "positivePart",
        1,
        xs -> (x=_cno_num(xs[1]); x===nothing ? nothing : Grounded(max(0.0, x)))
    )
    R["sigmoidNumber"] = _cno(
        "sigmoidNumber",
        1,
        xs -> (x=_cno_num(xs[1]); x===nothing ? nothing : Grounded(1.0/(1.0+exp(-x))))
    )
    R["absNumber"] = _cno(
        "absNumber", 1, xs -> (x=_cno_num(xs[1]); x===nothing ? nothing : Grounded(abs(x)))
    )
    R["expNumber"] = _cno(
        "expNumber", 1, xs -> (x=_cno_num(xs[1]); x===nothing ? nothing : Grounded(exp(x)))
    )
    R["roundNum"] = _cno(
        "roundNum",
        2,
        xs -> (
            x=_cno_num(xs[1]);
            d=_cno_int(xs[2]);
            (x===nothing||d===nothing) ? nothing : Grounded(round(x; digits=d))
        )
    )
    # (vec, index-vec) → scalar  ·  0-based indices (numpy)
    R["meanAtIndices"] = _cno(
        "meanAtIndices",
        2,
        xs -> begin
            v=_cno_fvec(xs[1])
            ...
            for c in xs[2].children
                (c isa Grounded && c.value isa Number && isinteger(c.value)) ||
                    return nothing
                push!(idx, Int(c.value))
            end
