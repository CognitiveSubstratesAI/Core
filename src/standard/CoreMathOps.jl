    ...
    Grounded(
        Operation(
            ...
            function (xs::Vector{Atom})
                ...
                (length(xs) == 1 && (x = _mnum(xs[1])) !== nothing) ||
                    return ExecRuntime(errmsg)
                r = try
                    f(Float64(x))
                catch e
        ...
        Operation(
            ...
            function (xs::Vector{Atom})
                ...
                (length(xs) == 1 && (x = _mnum(xs[1])) !== nothing) ||
                    return ExecRuntime(errmsg)
                ExecOk(
                    Atom[(x isa Integer ? false : test(x)) ? Sym("True") : Sym("False")]
                )
            end
        )
    )
...
const SQRT_MATH = _math_un_float(
    "sqrt-math", sqrt, "sqrt-math expects one argument: number"
)
const SIN_MATH = _math_un_float(
    "sin-math", sin, "sin-math expects one argument: input number"
)
const ASIN_MATH = _math_un_float(
    "asin-math", asin, "asin-math expects one argument: input number"
)
const COS_MATH = _math_un_float(
    "cos-math", cos, "cos-math expects one argument: input number"
)
const ACOS_MATH = _math_un_float(
    "acos-math", acos, "acos-math expects one argument: input number"
)
const TAN_MATH = _math_un_float(
    "tan-math", tan, "tan-math expects one argument: input number"
)
const ATAN_MATH = _math_un_float(
    "atan-math", atan, "atan-math expects one argument: input number"
)

const ABS_MATH = _math_un_preserve(
    "abs-math", abs, abs, "abs-math expects one argument: number"
)
const TRUNC_MATH = _math_un_preserve(
    "trunc-math", identity, trunc, "trunc-math expects one argument: input number"
)
const CEIL_MATH = _math_un_preserve(
    "ceil-math", identity, ceil, "ceil-math expects one argument: input number"
)
const FLOOR_MATH = _math_un_preserve(
    "floor-math", identity, floor, "floor-math expects one argument: input number"
)
...
const ROUND_MATH = _math_un_preserve(
    "round-math",
    identity,
    x -> round(x, RoundNearestTiesAway),
    "round-math expects one argument: input number"
)

const ISNAN_MATH = _math_un_bool(
    "isnan-math", isnan, "isnan-math expects one argument: input number"
)
const ISINF_MATH = _math_un_bool(
    "isinf-math", isinf, "isinf-math expects one argument: input number"
)
...
const POW_MATH = Grounded(
    Operation(
        "pow-math",
        function (xs::Vector{Atom})
            (_u = propagated_undefined(xs)) === nothing || return ExecOk(Atom[_u])   # WFS bottom contagious through strict ops
            length(xs) == 2 || return ExecRuntime(
                "pow-math expects two arguments: number (base) and number (power)"
            )
            b = _mnum(xs[1])
            p = _mnum(xs[2])     # assign unconditionally (not inside the && — keeps locals defined)
            (b === nothing || p === nothing) &&
                return ExecRuntime(
                    "pow-math expects two arguments: number (base) and number (power)"
                )
...
const LOG_MATH = Grounded(
    Operation(
        ...
        function (xs::Vector{Atom})
            ...
            r = try
                log(Float64(base), Float64(input))
            catch e
                e isa DomainError ? NaN : rethrow()
            end
            ExecOk(Atom[Grounded(r)])
        ...
        Operation(
            ...
            function (xs::Vector{Atom})
                ...
                (length(xs) == 1 && xs[1] isa Expression) ||
                    return ExecRuntime(
                        "$name expects one argument: an expression of numbers"
                    )
                ch = xs[1].children
                isempty(ch) && return ExecRuntime("Empty expression")
                all(c -> c isa Grounded && c.value isa Number, ch) ||
                    return ExecRuntime(
                        "Only numbers are allowed in expression: " * string(xs[1])
                    )
                best = ch[1]
                for c in @view ch[2:end]
                    better(c.value, best.value) && (best = c)
                end
                ExecOk(Atom[best])
            end
        )
...
function _register_core_math_ops!()
    R = TOKEN_REGISTRY
    R["sqrt-math"]=SQRT_MATH
    R["pow-math"]=POW_MATH
    R["abs-math"]=ABS_MATH
    R["log-math"]=LOG_MATH
    R["trunc-math"]=TRUNC_MATH
    R["ceil-math"]=CEIL_MATH
    R["floor-math"]=FLOOR_MATH
    R["round-math"]=ROUND_MATH
    R["sin-math"]=SIN_MATH
    R["asin-math"]=ASIN_MATH
    R["cos-math"]=COS_MATH
    R["acos-math"]=ACOS_MATH
    R["tan-math"]=TAN_MATH
    R["atan-math"]=ATAN_MATH
    R["isnan-math"]=ISNAN_MATH
    R["isinf-math"]=ISINF_MATH
    R["min-atom"]=MIN_ATOM
    R["max-atom"]=MAX_ATOM
end
